...
def run_apra_checks(data: dict) -> pd.DataFrame:
   ...
   # Step 4: Stress scenarios
    ...
    # AUS EQ and INTL EQ each shocked by -20%; other sleeves keep baseline returns
    ...
    # Bonds -5% (duration x 1.5%); REITs -5%; equities -2%; PE/VC -2%
...
# --- Figure 5.1 ---
def plot_figure_5_1(managers, benchmarks, taa_weights, saa_weights):
...
# --- Figure 5.2 ---
def plot_figure_5_2(managers, benchmarks, taa_weights, saa_weights):
    def weighted_return(returns_df, weights_dict):
        weights = pd.Series(weights_dict)
        return returns_df.mul(weights, axis=1).sum(axis=1)
